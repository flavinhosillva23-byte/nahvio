...
function checklistGuestOptions(selected=""){return `<option value="">Não definido</option>${state.guests.map(g=>`<option value="${g.id}" ${g.id===selected?"selected":""}>${esc(g.name)} - ${esc(g.side||"Ambos")}</option>`).join("")}`}
function checklistParticipantPicker(selected=[]){return ["Flávio","Nathália","Ambos"].map(side=>{const guests=state.guests.filter(g=>(g.side||"Ambos")===side);if(!guests.length)return "";return `<fieldset class="participant-group"><legend>${esc(side)}</legend>${guests.map(g=>`<label><input type="checkbox" name="taskParticipant" value="${g.id}" ${selected.includes(g.id)?"checked":""}><span>${esc(g.name)}</span></label>`).join("")}</fieldset>`}).join("")}
function renderProfessionalTasks(){
  ...
  if($("taskResponsibleFilter")){const current=$("taskResponsibleFilter").value;$("taskResponsibleFilter").innerHTML=`<option value="">Todos os responsáveis</option><option value="none" ${current==="none"?"selected":""}>Sem responsável</option>${state.guests.map(g=>`<option value="${g.id}" ${g.id===current?"selected":""}>${esc(g.name)}</option>`).join("")}`}
  const rows=state.tasks.filter(x=>(!f||(f==="open"&&!x.completed)||(f==="done"&&x.completed)||(f==="late"&&!x.completed&&x.due_date&&x.due_date<today))&&(!area||x.category===area)&&(!responsible||(responsible==="none"?!x.responsible_guest_id:x.responsible_guest_id===responsible))),done=state.tasks.filter(x=>x.completed).length,pct=state.tasks.length?Math.round(done/state.tasks.length*100):0;
  $("taskSummaryTop").innerHTML=`<span class="summary-pill">Progresso <strong>${pct}%</strong></span><span class="summary-pill">Total <strong>${state.tasks.length}</strong></span><span class="summary-pill">Concluídas <strong>${done}</strong></span><span class="summary-pill">Sem responsável <strong>${state.tasks.filter(x=>!x.responsible_guest_id).length}</strong></span><span class="summary-pill">Atrasadas <strong>${state.tasks.filter(x=>!x.completed&&x.due_date&&x.due_date<today).length}</strong></span>`;
  $("taskList").innerHTML=rows.map(x=>{const late=!x.completed&&x.due_date&&x.due_date<today,r=state.guests.find(g=>g.id===x.responsible_guest_id),names=(x.participant_guest_ids||[]).map(id=>state.guests.find(g=>g.id===id)?.name).filter(Boolean);return `<div class="task-row ${x.completed?"done":late?"late":""}"><input type="checkbox" ${x.completed?"checked":""} onchange="toggleTask('${x.id}',this.checked)"><div class="task-main"><div class="task-title-line"><strong>${esc(x.title)}</strong>${x.category?`<span class="task-area">${esc(x.category)}</span>`:""}${x.priority?`<span class="priority priority-${esc(x.priority.toLowerCase().replace(/\s+/g,"-"))}">${esc(x.priority)}</span>`:""}</div><small>${x.due_date?"Prazo: "+dateBR(x.due_date):"Sem prazo"}${late?" • Atrasada":""}</small><div class="task-people"><span><b>Responsável:</b> ${esc(r?.name||"Não definido")}</span><span><b>Participantes:</b> ${names.length?esc(names.join(", ")):"Nenhum selecionado"}</span></div>${x.notes?`<p>${esc(x.notes)}</p>`:""}</div><button class="secondary" onclick="taskModal('${x.id}')">Editar</button><button class="danger" onclick="removeRow('tasks','${x.id}')">Excluir</button></div>`}).join("")||`<p style="padding:16px">Nenhuma atividade nesta visão.</p>`;
}
window.taskModal=id=>{const x=state.tasks.find(t=>t.id===id)||{title:"",category:"Cerimônia",due_date:"",priority:"Normal",notes:"",responsible_guest_id:"",participant_guest_ids:[]};state.edit=id;modal(id?"Editar atividade":"Nova atividade",`<div class="form"><label class="full">Atividade<input id="cTitle" value="${esc(x.title)}"></label><label>Área<input id="cCategory" list="taskAreaList" value="${esc(x.category||"")}"><datalist id="taskAreaList">${["Civil","Cerimônia","Recepção","Música","Foto e vídeo","Áudio e vídeo","Indicadores","Alianças","Limpeza","Organização","Ensaio"].map(v=>`<option value="${v}">`).join("")}</datalist></label><label>Data<input id="cDue" type="date" value="${x.due_date||""}"></label><label>Prioridade<select id="cPriority">${["Baixa","Normal","Média","Alta","Muito alta"].map(p=>`<option ${p===(x.priority||"Normal")?"selected":""}>${p}</option>`).join("")}</select></label><label>Responsável<select id="cResponsible">${checklistGuestOptions(x.responsible_guest_id||"")}</select></label><label class="full">Participantes<div class="participant-help">Todos os convidados cadastrados estão disponíveis.</div><div class="participant-picker">${checklistParticipantPicker(x.participant_guest_ids||[])}</div></label><label class="full">Observações<textarea id="cNotes">${esc(x.notes||"")}</textarea></label></div><div class="actions"><button class="primary" onclick="saveTask()">Salvar atividade</button></div>`)};
window.saveTask=async()=>{const title=$("cTitle").value.trim();if(!title)return toast("Informe a atividade.");const obj={title,category:$("cCategory").value.trim()||null,due_date:$("cDue").value||null,priority:$("cPriority").value,responsible_guest_id:$("cResponsible").value||null,participant_guest_ids:[...document.querySelectorAll('input[name="taskParticipant"]:checked')].map(x=>x.value),notes:$("cNotes").value.trim()};const r=state.edit?await db.from("tasks").update(obj).eq("id",state.edit):await db.from("tasks").insert(obj);if(r.error)alert(r.error.message);else{$("modal").classList.add("hidden");toast(state.edit?"Atividade atualizada.":"Atividade adicionada.")}};
...
window.seedProfessionalChecklist=async()=>{
  ...
  const roles=[
    ...
    {title:"Discurso e celebração do casamento",category:"Cerimônia",priority:"Alta",completed:true,responsible:"Thiago Monteiro Ornaghi",people:["Thiago Monteiro Ornaghi"],notes:"Orador e celebrante definido: Thiago Monteiro Ornaghi."},
    {title:"Oração final da cerimônia",category:"Cerimônia",priority:"Alta",completed:true,responsible:"Nelson Aguiar",people:["Nelson Aguiar"],notes:"Responsável definido: Nelson Aguiar."},
    ...
    {title:"Equipe de áudio, vídeo e transmissão",category:"Áudio e vídeo",priority:"Alta",completed:true,people:["Paulo","Gustavo Gaspar"],notes:"Equipe definida: Paulo e Gustavo Gaspar. Inclui áudio, vídeo e transmissão via Zoom."},
    ...
    {title:"Limpeza antes da cerimônia",category:"Limpeza pré",priority:"Alta",completed:true,people:["Marcel","Marcelzinho","José Paulo","Gilberto","Flávio","Leandro","Heitor","Gustavo","Gabriel","Zittlau","Isabela"],notes:"Equipe: Marcel e Marcelzinho, José Paulo, Gilberto, Flávio, Leandro e Heitor, Gustavo e Gabriel, Zittlau e Isabela, além de quem desejar ajudar."},
    {title:"Limpeza e reorganização após a cerimônia",category:"Limpeza pós",priority:"Alta",completed:true,responsible:"Eduardo Rocha",people:["Eduardo Rocha","Henrik Nascimento","Érica","Carol","Fabyano","Shirley","Elias","Talita","João Victor","Laura","Lucas"],notes:"Responsáveis: Eduardo Rocha e Henrik Nascimento. Equipe: Érica e Carol, Fabyano e Shirley, Elias e Talita, João Victor e Laura, Lucas e Talita."},
    ...
    {title:"Cortejo - lado da Nathália",category:"Cortejo",priority:"Alta",completed:true,people:["Laureto","Márcia","Gabriel","Édelli","Gaspar","Solange","Maria Luiza"],notes:"Laureto e Márcia; Gabriel e Édelli; Gaspar e Solange; Maria Luiza."},
    {title:"Pais dos noivos",category:"Cerimônia",priority:"Alta",completed:true,people:["José Paulo","Iracema","Gilberto","Kelly"],notes:"Pais do noivo: José Paulo e Iracema. Pais da noiva: Gilberto e Kelly."},
  ...
  ],existing=new Set(state.tasks.map(x=>normalize(x.title))),rows=roles.filter(x=>!existing.has(normalize(x.title))).map(x=>{const r=findGuest(x.responsible);return {title:x.title,category:x.category,priority:x.priority,completed:x.completed,responsible_guest_id:r?.id||null,participant_guest_ids:ids(x.people||[]),notes:x.notes}});
  if(!rows.length)return toast("Checklist e funções já estão carregados.");const {error}=await db.from("tasks").insert(rows);if(error)alert(error.message);else toast(`${rows.length} funções e equipes adicionadas.`)
};
...
function exportChecklistProfessional(){
  ...
  let y=18;const addPage=()=>{d.addPage();y=18},write=(text,size=10,bold=false,indent=0)=>{d.setFont("helvetica",bold?"bold":"normal");d.setFontSize(size);const lines=d.splitTextToSize(String(text),178-indent);if(y+lines.length*5>282)addPage();d.text(lines,15+indent,y);y+=lines.length*5+2},person=id=>state.guests.find(g=>g.id===id)?.name||"Não definido",participants=x=>(x.participant_guest_ids||[]).map(person).filter(v=>v!=="Não definido").join(", ")||"Nenhum selecionado";
  ...
  write("ITENS CONCLUÍDOS",12,true);if(!done.length)write("Nenhum item concluído.");done.forEach(x=>{write(`[X] ${x.title}`,10,true);write(`Área: ${x.category||"Geral"} | Responsável: ${person(x.responsible_guest_id)}`,9,false,4);if((x.participant_guest_ids||[]).length)write(`Participantes: ${participants(x)}`,9,false,4)});y+=3;
  write("PRIORIDADES E PENDÊNCIAS",12,true);if(!pending.length)write("Nenhuma pendência.");
  const order={"Muito alta":0,"Alta":1,"Média":2,"Normal":3,"Baixa":4};pending.sort((a,b)=>(order[a.priority]??3)-(order[b.priority]??3)||(a.due_date||"9999").localeCompare(b.due_date||"9999")).forEach(x=>{write(`[ ] ${String(x.priority||"Normal").toUpperCase()} - ${x.title}`,10,true);write(`Área: ${x.category||"Geral"} | Prazo: ${x.due_date?dateBR(x.due_date):"Não definido"}`,9,false,4);write(`Responsável: ${person(x.responsible_guest_id)}`,9,false,4);write(`Participantes: ${participants(x)}`,9,false,4);if(x.notes)write(`Observações: ${x.notes}`,9,false,4)});
