...
function renderTasks(){
  if(typeof renderProfessionalTasks==="function")return renderProfessionalTasks();
...
function individualGuestPeople(){
  const overrides={
    "eduardo e familia":["Eduardo Rocha","Erica Rocha","Carol","Igor Thierry"],"wagner e familia":["Wagner Carvalho","Tatiane Carvalho","Murilo Carvalho","Marjorie Carvalho"],"andre, fran e familia":["André","Franciele","Luiza","Erick"],"diogo e thay":["Diogo","Thayara"],"gilson e mari":["Gilson","Mariana"],"creuza, neto e dari":["Creuza","Neto","Dariele"],"thiago e angela":["Thiago Ornaghi","Angela Ornaghi"],"nelson e cremilda":["Nelson Rodrigues","Cremilda Rodrigues"],"matheus e paulo":["Matheus Lopes","Paulo Lopes"],"gaspar, solange e gustavo":["José Gaspar","Solange Gaspar","Gustavo Gaspar"],"gabriel e edelli":["Gabriel Marchi","Edelli Marchi"],"gilberto, kelly e lulu":["Gilberto Rosa","Kelly Rosa","Maria Luiza Rosa"],"menabo e carol":["Willian Menabó","Carol Menabó"],"lucas e thalita":["Lucas Zanardi","Talita Zanardi"],"jose e iracema":["José Paulo","Iracema"],"zittlau e isa":["Zittlau","Isabela"],"marcel, pri e marcelzinho":["Marcel","Priscila","Marcelzinho"],"luiz e fer":["Luiz","Fernanda"],"adriano":["Adriano Fonseca"],"henrik":["Henrik Nascimento"],"rodolfo":["Rodolfo Ferreira"]
  },norm=v=>String(v||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim();
  return state.guests.flatMap(g=>{const names=overrides[norm(g.name)]||String(g.name||"").replace(/\s+e\s+fam[ií]lia$/i,"").split(/\s*,\s*|\s+e\s+/i).map(x=>x.trim()).filter(Boolean);return names.map(name=>({name,side:g.side||"Ambos",guestId:g.id,source:g.name}))}).sort((a,b)=>a.name.localeCompare(b.name,"pt-BR"));
}
function checklistGuestOptions(selected=""){return `<option value="">Não definido</option>${individualGuestPeople().map(p=>`<option value="${esc(p.name)}" ${p.name===selected?"selected":""}>${esc(p.name)} - ${esc(p.side)}</option>`).join("")}`}
function checklistParticipantPicker(selected=[]){const people=individualGuestPeople();return ["Flávio","Nathália","Ambos"].map(side=>{const rows=people.filter(p=>p.side===side);if(!rows.length)return "";return `<fieldset class="participant-group"><legend>${esc(side)}</legend>${rows.map(p=>`<label title="Convite: ${esc(p.source)}"><input type="checkbox" name="taskParticipant" value="${esc(p.name)}" ${selected.includes(p.name)?"checked":""}><span>${esc(p.name)}</span></label>`).join("")}</fieldset>`}).join("")}
function renderProfessionalTasks(){
  const f=$("taskFilter").value,area=$("taskAreaFilter")?.value||"",responsible=$("taskResponsibleFilter")?.value||"",today=new Date().toISOString().slice(0,10),areas=[...new Set(state.tasks.map(x=>x.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"pt-BR"));
  if($("taskAreaFilter")){const current=$("taskAreaFilter").value;$("taskAreaFilter").innerHTML=`<option value="">Todas as áreas</option>${areas.map(v=>`<option ${v===current?"selected":""}>${esc(v)}</option>`).join("")}`}
  if($("taskResponsibleFilter")){const current=$("taskResponsibleFilter").value;$("taskResponsibleFilter").innerHTML=`<option value="">Todos os responsáveis</option><option value="none" ${current==="none"?"selected":""}>Sem responsável</option>${individualGuestPeople().map(p=>`<option value="${esc(p.name)}" ${p.name===current?"selected":""}>${esc(p.name)}</option>`).join("")}`}
  const rows=state.tasks.filter(x=>(!f||(f==="open"&&!x.completed)||(f==="done"&&x.completed)||(f==="late"&&!x.completed&&x.due_date&&x.due_date<today))&&(!area||x.category===area)&&(!responsible||(responsible==="none"?!x.responsible_name:x.responsible_name===responsible))),done=state.tasks.filter(x=>x.completed).length,pct=state.tasks.length?Math.round(done/state.tasks.length*100):0;
  $("taskSummaryTop").innerHTML=`<span class="summary-pill">Progresso <strong>${pct}%</strong></span><span class="summary-pill">Total <strong>${state.tasks.length}</strong></span><span class="summary-pill">Concluídas <strong>${done}</strong></span><span class="summary-pill">Sem responsável <strong>${state.tasks.filter(x=>!x.responsible_name).length}</strong></span><span class="summary-pill">Atrasadas <strong>${state.tasks.filter(x=>!x.completed&&x.due_date&&x.due_date<today).length}</strong></span>`;
  $("taskList").innerHTML=rows.map(x=>{const late=!x.completed&&x.due_date&&x.due_date<today,names=x.participant_names||[];return `<div class="task-row ${x.completed?"done":late?"late":""}"><input type="checkbox" ${x.completed?"checked":""} onchange="toggleTask('${x.id}',this.checked)"><div class="task-main"><div class="task-title-line"><strong>${esc(x.title)}</strong>${x.category?`<span class="task-area">${esc(x.category)}</span>`:""}${x.priority?`<span class="priority priority-${esc(x.priority.toLowerCase().replace(/\s+/g,"-"))}">${esc(x.priority)}</span>`:""}</div><small>${x.due_date?"Prazo: "+dateBR(x.due_date):"Sem prazo"}${late?" • Atrasada":""}</small><div class="task-people"><span><b>Responsável:</b> ${esc(x.responsible_name||"Não definido")}</span><span><b>Participantes:</b> ${names.length?esc(names.join(", ")):"Nenhum selecionado"}</span></div>${x.notes?`<p>${esc(x.notes)}</p>`:""}</div><button class="secondary" onclick="taskModal('${x.id}')">Editar</button><button class="danger" onclick="removeRow('tasks','${x.id}')">Excluir</button></div>`}).join("")||`<p style="padding:16px">Nenhuma atividade nesta visão.</p>`;
}
window.taskModal=id=>{const x=state.tasks.find(t=>t.id===id)||{title:"",category:"Cerimônia",due_date:"",priority:"Normal",notes:"",responsible_name:"",participant_names:[]};state.edit=id;modal(id?"Editar atividade":"Nova atividade",`<div class="form"><label class="full">Atividade<input id="cTitle" value="${esc(x.title)}"></label><label>Área<input id="cCategory" list="taskAreaList" value="${esc(x.category||"")}"><datalist id="taskAreaList">${["Civil","Cerimônia","Recepção","Música","Foto e vídeo","Áudio e vídeo","Indicadores","Alianças","Limpeza pré","Limpeza pós","Organização","Cortejo","Ensaio"].map(v=>`<option value="${v}">`).join("")}</datalist></label><label>Data<input id="cDue" type="date" value="${x.due_date||""}"></label><label>Prioridade<select id="cPriority">${["Baixa","Normal","Média","Alta","Muito alta"].map(p=>`<option ${p===(x.priority||"Normal")?"selected":""}>${p}</option>`).join("")}</select></label><label>Responsável<select id="cResponsible">${checklistGuestOptions(x.responsible_name||"")}</select></label><label class="full">Participantes<div class="participant-help">As pessoas foram separadas individualmente, mesmo quando pertencem ao mesmo convite.</div><div class="participant-picker">${checklistParticipantPicker(x.participant_names||[])}</div></label><label class="full">Observações<textarea id="cNotes">${esc(x.notes||"")}</textarea></label></div><div class="actions"><button class="primary" onclick="saveTask()">Salvar atividade</button></div>`)};
window.saveTask=async()=>{const title=$("cTitle").value.trim();if(!title)return toast("Informe a atividade.");const obj={title,category:$("cCategory").value.trim()||null,due_date:$("cDue").value||null,priority:$("cPriority").value,responsible_name:$("cResponsible").value||null,participant_names:[...document.querySelectorAll('input[name="taskParticipant"]:checked')].map(x=>x.value),notes:$("cNotes").value.trim()};const r=state.edit?await db.from("tasks").update(obj).eq("id",state.edit):await db.from("tasks").insert(obj);if(r.error)alert(r.error.message);else{$("modal").classList.add("hidden");toast(state.edit?"Atividade atualizada.":"Atividade adicionada.")}};
window.seedProfessionalChecklist=async()=>{const templates=[["Cronograma completo e operacional finalizados","Organização","Alta",true],["Sequência das entradas definida","Cerimônia","Alta",true],["Presidente, orador e oração final definidos","Cerimônia","Alta",true],["Votos definidos no esboço do Thiago","Cerimônia","Alta",true],["Cerimonialista contratada com equipe própria","Organização","Alta",true],["Fotógrafo, filmagem e transmissão via Zoom confirmados","Foto e vídeo","Alta",true],["Equipe de áudio e vídeo definida","Áudio e vídeo","Alta",true],["Indicadores definidos","Indicadores","Alta",true],["Equipes de limpeza e reorganização definidas","Limpeza","Alta",true],["Responsáveis pela reorganização definidos","Organização","Alta",true],["Reunião às 19h confirmada","Cerimônia","Alta",true],["Salão totalmente livre até 18h10","Organização","Alta",true],["Definir as testemunhas civis","Civil","Muito alta",false],["Escolher o cântico final","Música","Alta",false],["Fechar a playlist completa","Música","Alta",false],["Definir responsável pelo Heitor e pelas alianças","Alianças","Média",false],["Marcar os lugares reservados","Cerimônia","Média",false],["Realizar o ensaio geral com todos os envolvidos","Ensaio","Baixa",false]],existing=new Set(state.tasks.map(x=>x.title.trim().toLowerCase())),rows=templates.filter(x=>!existing.has(x[0].toLowerCase())).map(x=>({title:x[0],category:x[1],priority:x[2],completed:x[3],participant_guest_ids:[]}));if(!rows.length)return toast("O checklist profissional já está carregado.");const {error}=await db.from("tasks").insert(rows);if(error)alert(error.message);else toast(`${rows.length} atividades adicionadas.`)};
[$("taskAreaFilter"),$("taskResponsibleFilter")].filter(Boolean).forEach(x=>x.oninput=renderTasks);
if($("seedChecklistBtn"))$("seedChecklistBtn").onclick=()=>{if(confirm("Adicionar os itens do checklist profissional? Itens com o mesmo nome não serão duplicados."))seedProfessionalChecklist()};
const seedBaseChecklist=window.seedProfessionalChecklist;
window.seedProfessionalChecklist=async()=>{
  const normalize=v=>String(v||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase(),findGuest=name=>{const q=normalize(name),first=q.split(/\s+/)[0];return state.guests.find(g=>normalize(g.name)===q)||state.guests.find(g=>normalize(g.name).includes(q))||state.guests.find(g=>normalize(g.name).split(/\s+/).includes(first))},ids=names=>[...new Set(names.map(findGuest).filter(Boolean).map(g=>g.id))];
  await seedBaseChecklist();
  const roles=[
    {title:"Presidência da cerimônia",category:"Cerimônia",priority:"Alta",completed:true,responsible:"Wagner Carvalho",people:["Wagner Carvalho"],notes:"Presidente definido: Wagner Carvalho."},
    {title:"Discurso e celebração do casamento",category:"Cerimônia",priority:"Alta",completed:true,responsible:"Thiago Ornaghi",people:["Thiago Ornaghi"],notes:"Orador e celebrante definido: Thiago Ornaghi."},
    {title:"Oração final da cerimônia",category:"Cerimônia",priority:"Alta",completed:true,responsible:"Nelson Rodrigues",people:["Nelson Rodrigues"],notes:"Responsável definido: Nelson Rodrigues."},
    {title:"Testemunhas do casamento civil",category:"Civil",priority:"Muito alta",completed:false,people:[],notes:"Definir as testemunhas civis e confirmar a documentação necessária."},
    {title:"Entrada e cuidado das alianças",category:"Alianças",priority:"Média",completed:false,people:["Heitor"],notes:"Heitor fará a entrada com as alianças. Definir um adulto responsável por acompanhá-lo e guardar as alianças."},
    {title:"Equipe de indicadores",category:"Indicadores",priority:"Alta",completed:true,people:["Adriano Fonseca","Eduardo Rocha","Igor Thierry","Henrik Nascimento","Matheus Lopes","Rodolfo Ferreira"],notes:"Equipe: Adriano Fonseca, Eduardo Rocha, Igor Thierry, Henrik Nascimento, Matheus Lopes e Rodolfo Ferreira."},
    {title:"Equipe de áudio, vídeo e transmissão",category:"Áudio e vídeo",priority:"Alta",completed:true,people:["Paulo Lopes","Gustavo Gaspar"],notes:"Equipe definida: Paulo Lopes e Gustavo Gaspar. Inclui áudio, vídeo e transmissão via Zoom."},
    {title:"Fotografia e filmagem",category:"Foto e vídeo",priority:"Alta",completed:true,people:[],notes:"Fotógrafo e filmagem confirmados. Cadastrar os nomes dos profissionais, se desejado."},
    {title:"Cerimonialista e equipe de apoio",category:"Organização",priority:"Alta",completed:true,people:[],notes:"Cerimonialista contratada com equipe própria. Cadastrar o nome e o contato principal."},
    {title:"Limpeza antes da cerimônia",category:"Limpeza pré",priority:"Alta",completed:true,people:["Marcel","Marcelzinho","José Paulo","Gilberto Rosa","Flávio","Leandro","Heitor","Gustavo Gaspar","Gabriel Marchi","Zittlau","Isabela"],notes:"Equipe: Marcel e Marcelzinho, José Paulo, Gilberto Rosa, Flávio, Leandro e Heitor, Gustavo Gaspar e Gabriel Marchi, Zittlau e Isabela, além de quem desejar ajudar."},
    {title:"Limpeza e reorganização após a cerimônia",category:"Limpeza pós",priority:"Alta",completed:true,responsible:"Eduardo Rocha",people:["Eduardo Rocha","Henrik Nascimento","Erica Rocha","Carol","Fabyano","Shirley","Elias","Talita","João Victor","Laura","Lucas Zanardi","Talita Zanardi"],notes:"Responsáveis: Eduardo Rocha e Henrik Nascimento. Equipe: Erica Rocha e Carol, Fabyano e Shirley, Elias e Talita, João Victor e Laura, Lucas Zanardi e Talita Zanardi."},
    {title:"Liberação completa do salão até 18h10",category:"Organização",priority:"Muito alta",completed:false,responsible:"Eduardo Rocha",people:["Eduardo Rocha","Henrik Nascimento"],notes:"Eduardo e Henrik coordenam a reorganização. O Salão deverá estar completamente livre até 18h10 por causa da reunião das 19h."},
    {title:"Cortejo - lado do Flávio",category:"Cortejo",priority:"Alta",completed:true,people:["Leandro","Naiara","Fernanda","Luiz","Marcel","Priscila","Marcelzinho","Diane","Rogério"],notes:"Leandro e Naiara; Fernanda e Luiz; Marcel, Priscila e Marcelzinho; Diane e Rogério."},
    {title:"Cortejo - lado da Nathália",category:"Cortejo",priority:"Alta",completed:true,people:["Laureto","Márcia","Gabriel Marchi","Edelli Marchi","José Gaspar","Solange Gaspar","Maria Luiza Rosa"],notes:"Laureto e Márcia; Gabriel Marchi e Edelli Marchi; José Gaspar e Solange Gaspar; Maria Luiza Rosa."},
    {title:"Pais dos noivos",category:"Cerimônia",priority:"Alta",completed:true,people:["José Paulo","Iracema","Gilberto Rosa","Kelly Rosa"],notes:"Pais do noivo: José Paulo e Iracema. Pais da noiva: Gilberto Rosa e Kelly Rosa."},
    {title:"Ensaio geral com todos os envolvidos",category:"Ensaio",priority:"Baixa",completed:false,people:[],notes:"Reunir cortejo, presidente, orador, alianças, cerimonialista, indicadores e equipes técnicas."},
    {title:"Definição e marcação dos lugares reservados",category:"Recepção",priority:"Média",completed:false,people:[],notes:"Marcar lugares dos pais, participantes do cortejo, idosos e demais reservas."},
    {title:"Playlist completa da recepção",category:"Música",priority:"Alta",completed:false,people:[],notes:"Revisar a ordem, duração e adequação das músicas."},
    {title:"Cântico final da cerimônia",category:"Música",priority:"Alta",completed:false,people:[],notes:"Escolher e confirmar o cântico final."}
  ],existing=new Set(state.tasks.map(x=>normalize(x.title))),rows=roles.filter(x=>!existing.has(normalize(x.title))).map(x=>({title:x.title,category:x.category,priority:x.priority,completed:x.completed,responsible_name:x.responsible||null,participant_names:x.people||[],notes:x.notes}));
  if(rows.length){const {error}=await db.from("tasks").insert(rows);if(error)return alert(error.message)}
  const updates=await Promise.all(roles.map(x=>db.from("tasks").update({responsible_name:x.responsible||null,participant_names:x.people||[],notes:x.notes}).eq("title",x.title)));if(updates.some(x=>x.error))return alert("Não foi possível atualizar todas as funções.");toast(rows.length?`${rows.length} funções e equipes adicionadas.`:"Funções e participantes atualizados.")
};
...
function exportChecklistProfessional(){
  const {jsPDF}=window.jspdf||{};if(!jsPDF)return toast("Não foi possível iniciar o PDF.");
  const d=new jsPDF({unit:"mm",format:"a4"}),done=state.tasks.filter(x=>x.completed),pending=state.tasks.filter(x=>!x.completed),pct=state.tasks.length?Math.round(done.length/state.tasks.length*100):0;
  let y=18;const addPage=()=>{d.addPage();y=18},write=(text,size=10,bold=false,indent=0)=>{d.setFont("helvetica",bold?"bold":"normal");d.setFontSize(size);const lines=d.splitTextToSize(String(text),178-indent);if(y+lines.length*5>282)addPage();d.text(lines,15+indent,y);y+=lines.length*5+2},person=x=>x.responsible_name||"Não definido",participants=x=>(x.participant_names||[]).join(", ")||"Nenhum selecionado";
  d.setFillColor(18,63,51);d.rect(0,0,210,34,"F");d.setTextColor(255,255,255);d.setFont("helvetica","bold");d.setFontSize(17);d.text("CHECKLIST DO CASAMENTO",15,17);d.setFontSize(12);d.text("FLÁVIO & NATHÁLIA",15,25);d.setTextColor(30,35,32);y=43;
  write("STATUS GERAL",12,true);write(`Progresso calculado: ${pct}% (${done.length} de ${state.tasks.length} atividades concluídas).`);write(pending.length?`Restam ${pending.length} atividades para concluir.`:"Todas as atividades foram concluídas.");y+=3;
  write("ITENS CONCLUÍDOS",12,true);if(!done.length)write("Nenhum item concluído.");done.forEach(x=>{write(`[X] ${x.title}`,10,true);write(`Área: ${x.category||"Geral"} | Responsável: ${person(x)}`,9,false,4);if((x.participant_names||[]).length)write(`Participantes: ${participants(x)}`,9,false,4)});y+=3;
  write("PRIORIDADES E PENDÊNCIAS",12,true);if(!pending.length)write("Nenhuma pendência.");
  const order={"Muito alta":0,"Alta":1,"Média":2,"Normal":3,"Baixa":4};pending.sort((a,b)=>(order[a.priority]??3)-(order[b.priority]??3)||(a.due_date||"9999").localeCompare(b.due_date||"9999")).forEach(x=>{write(`[ ] ${String(x.priority||"Normal").toUpperCase()} - ${x.title}`,10,true);write(`Área: ${x.category||"Geral"} | Prazo: ${x.due_date?dateBR(x.due_date):"Não definido"}`,9,false,4);write(`Responsável: ${person(x)}`,9,false,4);write(`Participantes: ${participants(x)}`,9,false,4);if(x.notes)write(`Observações: ${x.notes}`,9,false,4)});
  const pages=d.getNumberOfPages();for(let i=1;i<=pages;i++){d.setPage(i);d.setFontSize(8);d.setTextColor(100);d.text(`Nahvio - Página ${i} de ${pages}`,195,290,{align:"right"})}d.save("nahvio_checklist_casamento.pdf");toast("PDF do checklist gerado.");
}
function currentPagePdfData(id){
  const maps={
    dashboard:["Visão geral",[`Casamento: ${state.settings.coupleNames}`,`Data: ${dateBR(state.settings.weddingDate)} às ${state.settings.weddingTime}`,`Convidados cadastrados: ${state.guests.length}`,`Confirmados: ${state.guests.filter(x=>x.rsvp_status==="Confirmado").length}`,`Tarefas concluídas: ${state.tasks.filter(x=>x.completed).length} de ${state.tasks.length}`]],
    guests:["Lista de convidados",state.guests.map(g=>`${g.name} | Lado: ${g.side} | Pessoas: ${people(g)} | ${g.rsvp_status} | Mesa: ${g.table_name||"Não definida"}`)],
    tables:["Organização das mesas",state.tables.map(t=>`${t.name} | Capacidade: ${t.capacity} | Lugares ocupados: ${tableUsage(t)}`)],
    finance:["Controle financeiro",state.finance.map(x=>`${x.description} | ${x.category} | Previsto: ${money(x.planned)} | Pago: ${money(x.paid)} | Restante: ${money(n(x.planned)-n(x.paid))}`)],
    gifts:["Presentes",state.gifts.map(x=>`${x.item} | Quem deu: ${x.given_by||"Não informado"} | ${x.received?"Recebido":"Pendente"} | Agradecimento: ${x.thanked?"Enviado":"Pendente"}`)],
    documents:["Documentos",state.documents.map(x=>`${x.title} | ${x.category} | ${x.provider||"Sem fornecedor"} | ${x.notes||""}`)],
    reports:["Resumo geral",[`Convidados: ${state.guests.length}`,`Confirmações: ${state.guests.filter(x=>x.rsvp_status==="Confirmado").length}`,`Mesas: ${state.tables.length}`,`Tarefas: ${state.tasks.length}`,`Tarefas concluídas: ${state.tasks.filter(x=>x.completed).length}`,`Lançamentos financeiros: ${state.finance.length}`]],
    "shower-dashboard":["Chá de cozinha - Visão geral",[`Evento: ${state.showerSettings.eventName}`,`Data: ${dateBR(state.showerSettings.eventDate)} às ${state.showerSettings.eventTime}`,`Convidados: ${state.showerGuests.length}`,`Confirmados: ${state.showerGuests.filter(x=>x.rsvp_status==="Confirmado").length}`]],
    "shower-guests":["Chá de cozinha - Convidados",state.showerGuests.map(x=>`${x.full_name} | Pessoas: ${1+n(x.companions)} | ${x.rsvp_status} | Telefone: ${x.phone||"Não informado"}`)],
    "shower-gifts":["Chá de cozinha - Presentes",state.showerGifts.map(x=>`${x.item} | ${x.category||"Geral"} | ${x.status} | ${x.given_by||"Não informado"}`)],
    "shower-tasks":["Chá de cozinha - Checklist",state.showerTasks.map(x=>`${x.completed?"[X]":"[ ]"} ${x.title} | ${x.priority} | ${x.due_date?dateBR(x.due_date):"Sem prazo"}`)],
    "shower-games":["Chá de cozinha - Brincadeiras",state.showerGames.map(x=>`${x.position_index||"-"}. ${x.title} | ${x.status} | Responsável: ${x.responsible||"Não definido"}`)],
    "shower-finance":["Chá de cozinha - Financeiro",state.showerFinance.map(x=>`${x.description} | Previsto: ${money(x.planned)} | Pago: ${money(x.paid)}`)],
    "shower-schedule":["Chá de cozinha - Cronograma",state.showerSchedule.map(x=>`${x.time||"--:--"} | ${x.title} | ${x.notes||""}`)],
    ceremony:["Cerimônia",state.tasks.map(x=>`${x.completed?"[X]":"[ ]"} ${x.title}`)],
    assistant:["Resumo de consulta",[$("assistantAnswer")?.textContent||"Nenhuma consulta realizada."]],
    settings:["Configurações do casamento",[`Casal: ${state.settings.coupleNames}`,`Data: ${dateBR(state.settings.weddingDate)}`,`Horário: ${state.settings.weddingTime}`,`Limite de convidados: ${state.settings.guestLimit}`]],
    "shower-settings":["Configurações do chá",[`Evento: ${state.showerSettings.eventName}`,`Data: ${dateBR(state.showerSettings.eventDate)}`,`Horário: ${state.showerSettings.eventTime}`,`Local: ${state.showerSettings.location||"Não definido"}`]]
  };return maps[id]||["Relatório Nahvio",["Esta área não possui registros próprios para exportação."]]
}
function exportCurrentPagePdf(){const id=document.querySelector(".page.active")?.id||"dashboard";if(id==="tasks"||id==="ceremony")return exportChecklistProfessional();const [title,lines]=currentPagePdfData(id),d=pdfDoc(`Nahvio - ${title}`,"Relatório organizado da área selecionada");if(!d)return toast("Não foi possível iniciar o PDF.");savePdf(d,lines.length?lines:["Nenhum registro encontrado."],`nahvio_${id.replace(/-/g,"_")}.pdf`);toast("PDF gerado.")}
...
if($("exportCurrentPageBtn"))$("exportCurrentPageBtn").onclick=exportCurrentPagePdf;
